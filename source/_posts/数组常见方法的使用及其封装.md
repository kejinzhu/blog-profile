...
- 检测数组成员在数组中第一次出现的索引位置； 判断当前项是否在数组中存在；如果不存在，返回-1；
- 需要参数
- 返回在数组中第一次出现的索引；
- 原有数组不发生改变
...
- 检测数组成员在数组中最后一次出现的索引位置； 判断当前项是否在数组中存在；如果不存在，返回-1；
- 需要参数
- 返回在数组中最后一次出现的索引；
- 原有数组不发生改变；
...
Array.prototype.myFilter = function(fn) {
    ...
    for (var i = 0; i < this.length; i++) {
        var cur = this[i];
        try {
            result = fn.call(null, cur, i, this);
        } catch (e) {
            throw new Error(fn + ' is not a function');
        }
        if (result) {
...
## find 
查找；从所到右一次进行查找；找到符合条件的第一项，并且把这一项返回；结束整个函数
...
Array.prototype.myFind = function(fn) {
    ...
    for (var i = 0; i < this.length; i++) {
        var cur = this[i];
        try {
            result = fn.call(null, cur, i, this) ? true : undefined;
        } catch (e) {
            throw new Error(fn + ' is not a function');
        }
        if (result) {
...
## some 
返回一个布尔值；如果有符合条件的返回true，如果没有找到符合条件的，返回是false
...
Array.prototype.mySome = function(fn) {
    ...
    for (var i = 0; i < this.length; i++) {
        var cur = this[i];
        try {
            result = fn.call(null, cur, i, this) ? true : false;
        } catch (e) {
            throw new Error(fn + ' is not a function');
        }
    }
...
## every 
返回一个布尔值；true或false；必须要求每一项都返回true，整体结果返回true；只要有一个是false，整体结果就是false
...
## includes 
包含；检测是否包含某一项；如果包含返回true，不包含返回false；
...
```javascript
Array.prototype.myReduce = function(fn, param) {
    var prev;
    for (var i = 0; i < this.length; i++) {
        var cur = this[i];
        if (i == 0) {
        ...
        try {
            prev = fn.call(null, prev, cur, i, this);
